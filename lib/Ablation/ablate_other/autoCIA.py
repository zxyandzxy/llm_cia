...
from lib.Ablation.ablate_other import llm_utils, pro_interface

Qwen_RESULT_DIR = 'D:\science_research\change_impact_analysis\llm_cia\Ablation_Results'

class AutoCIA(llm_utils.OpenAIEngine):
    def __init__(self, model_type, system_file, project_name, commit_num, ablate_func):
        super().__init__(model_type)
        self._system_file = system_file
        self._project_name = project_name
        self._commit_num = commit_num
        self._ablate_func = ablate_func
    ...
    def startup(self):
        ...
        first_respose = self.get_response(self.messages, self._ablate_func)
    ...
    def step(self, function_call_mode="none"):
        ...
        response = self.get_response(prompt_messages, self._ablate_func)
...
def getQwenResult(model, project_name, commit_num, ablate_func):
    svn_update_to_revision(commit_num, project2root[project_name])
    ad = AutoCIA(model, './prompts/system_msg.txt', project_name, commit_num, ablate_func)
    ...
    func_num = funcName2funcNum(ablate_func)
    cost_time_path = os.path.join("D:\science_research\change_impact_analysis\llm_cia\Ablation_Results", func_num, "LLM_CIA_COST", f"{project_name}.txt")

    with open(cost_time_path, 'a', encoding='utf-8') as file:
        file.write(f"{commit_num} : {end - start:.5f} s\n")
    file_path = os.path.join(Qwen_RESULT_DIR, func_num, project_name, str(commit_num) + '.json')
...
def funcName2funcNum(ablate_func):
    if ablate_func == "get_coupling_dependencies":
        return "func1"
    elif ablate_func == "get_conceptual_coupling":
        return "func3"
    elif ablate_func == "get_call_graph":
        return "func4"
    elif ablate_func == "get_origin_code":
        return "func5"

if __name__ == '__main__':
    project = ["freecol", "hsqldb", "JAMWiki", "jEdit", "JHotDraw", "Makagiga", "OmegaT"]
    ablate_func = "get_coupling_dependencies" # get_coupling_dependencies, get_conceptual_coupling, get_call_graph, get_origin_code
    for project_name in project:
        commit_file_path = os.path.join("D:\science_research\change_impact_analysis\llm_cia\\ablate_commit", f"{project_name}.txt")
        ...
        for i in sample_list:
            output_path = os.path.join(Qwen_RESULT_DIR, funcName2funcNum(ablate_func), project_name, f"{str(i)}.json")
            if os.path.exists(output_path):
                continue
            getQwenResult("qwen-plus-latest", project_name, i, ablate_func)