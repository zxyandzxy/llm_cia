    ...
    with os.scandir(parent_folder) as entries:
        for entry in entries:
            if entry.is_dir():
                ...
                change_set = []
                for tra in cp_result:
                    support = 0

                    if cp_result[tra] is True:
                        support += 1
    ...
    with os.scandir(parent_folder) as entries:
        for entry in entries:
            if entry.is_dir():
                ...
                combined = func1_result + func2_result + func4_result
    ...
    with os.scandir(parent_folder) as entries:
        for entry in entries:
            if entry.is_dir():
                ...
                # Merge the three arrays and count the number of occurrences of each tra_name
                combined = func1_result + func2_res + func4_res
                counter = Counter(combined)
...
if __name__ == "__main__":
    ...
    for project_name in project:
        # Program1
        average_metrics = Program1(parent_folder)
        ...
        # Program2
        average_metrics = Program2(parent_folder)
        for k in average_metrics:
            print("k = " + str(k) + "-" * 100)
            print("recall: " + "{:.4f}".format(average_metrics[k]["average_recall"]))
            print("precision: " + "{:.4f}".format(average_metrics[k]["average_precision"]))
        ...
        # Program3
        average_metrics = Program3(parent_folder)
        for k in average_metrics:
            print("k = " + str(k) + "-" * 100)
            print("recall: " + "{:.4f}".format(average_metrics[k]["average_recall"]))
            print("precision: " + "{:.4f}".format(average_metrics[k]["average_precision"]))
